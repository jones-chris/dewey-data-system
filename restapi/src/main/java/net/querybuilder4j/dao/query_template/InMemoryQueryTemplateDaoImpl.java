...
public class InMemoryQueryTemplateDaoImpl implements QueryTemplateDao {
    ...
    private final Set<SelectStatement> queryTemplates;

    public InMemoryQueryTemplateDaoImpl() {
        this(new HashSet<>());
    }

    public InMemoryQueryTemplateDaoImpl(Set<SelectStatement> queryTemplates) {
        this.queryTemplates = Objects.requireNonNullElseGet(queryTemplates, HashSet::new);
    }
...
//    }
    ...
    public Set<String> listNames(String databaseName) {
        return this.queryTemplates.stream()
                .filter(template -> databaseName.equals(template.getDatabase().getDatabaseName()))
