    ...
    for (let index in criteria) {
        ...
        if (criterion.metadata.id === idToDelete) {
            ...
            criterionToDelete.childCriteria.forEach(childCriterion => {
                if (parentCriterion === undefined || parentCriterion === null) {
 ...
 */
export const setCriterionMetadata = (criteria, nextId = 0) => {
    criteria.forEach(criterion => {
        // Set id.
        ...
        // Set level.
        if (criterion.parentCriterion === undefined || criterion.parentCriterion === null) {
...
export const replaceParentCriterionIds = (criteria) => {
    ...
    return flattenedCriteria.map(criterion => {
        ...
        if (criterionClone.parentCriterion !== undefined && criterionClone.parentCriterion !== null) {
...
/**
 * Unflattens/nests the criteria array.
 *
 * @param criteria The array of criteria to unflatten/nest.
 * @param unflattenedCriteriaHolder The array that holds the unflattened/nested criteria.
 * @returns An unflattened/nested array of criterion.
 */
export const unflattenCriteria = (criteria, unflattenedCriteriaHolder) => {
    criteria.forEach(criterion => {
        // Make sure criterion object has children property.
        if (criterion.childCriteria === undefined) {
            criterion.childCriteria = [];
        }

        // If criterion is a root, then push to the holder.
        if (criterion.parentId === undefined) {
            if (criterion.metadata === undefined) {
                criterion.metadata = {
                    level: 0
                }
            }

            unflattenedCriteriaHolder.push(criterion);
        }
        // If criterion has a parent, then find parent and add criterion to the parent's children.
        else {
            let parentId = criterion.parentId;
            let parentCriterion = criteria.filter(criterion => criterion.id === parentId)[0];

            criterion.parentCriterion = parentCriterion;
            ...
            if (parentCriterion.childCriteria === undefined) {
                parentCriterion.childCriteria = [criterion];
            }
            else {
                parentCriterion.childCriteria.push(criterion);
            }
        }
    });

    return unflattenedCriteriaHolder;
}
